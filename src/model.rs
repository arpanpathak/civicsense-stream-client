// YOLOv8 detection model, ported from candle's official example
// https://github.com/huggingface/candle/tree/0.7.2/candle-examples/examples/yolo-v8
...
impl YoloV8 {
    /// Load the YOLOv8n model from a safetensors file.
    pub fn load_from_file(path: &str, device: &candle_core::Device) -> Result<Self> {
        let data = std::fs::read(path)?;
        let vb = VarBuilder::from_slice_safetensors(&data, DType::F32, device)?;
        Self::load(vb, Multiples::n(), 80)
    }
